...
class TestBlanca {
	...
	static public Question crearPregunta(int i, String a, int x, Pronostico Pa, Event Ea, ArrayList<Pronostico> listaPa) {
	...
	void test2() {
		try {
			...
			assertNull(db.find(Event.class, E2.getEventNumber()));
	...
	void test3() {
		try {
			...
			assertNull(db.find(Event.class, E3.getEventNumber()));
	...
	void test4() {
		try {
			...
			assertNull(db.find(Event.class, E4.getEventNumber()));
